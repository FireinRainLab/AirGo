...
func GetRouteList(c *gin.Context) {
	...
	uID, ok := c.Get("uID")
	if !ok {
		response.Fail("获取菜单参数错误", nil, c)
		return
	}
	//格式转化
	uIdNew, _ := uID.(int)
	//查询uId对应的角色
	roleIds, err := service.FindRoleIdsByuId(uIdNew)
	if err != nil {
		global.Logrus.Error("角色查询错误", err.Error())
		response.Fail("角色查询错误"+err.Error(), nil, c)
		return
	}
	// 角色Ids对应的route Ids
	...
	if err != nil {
		...
		response.Fail("GetRouteIdsByRoleIds err"+err.Error(), nil, c)
		return
	}
	...
	if err != nil {
		...
		response.Fail("GetRouteSliceByRouteIds err"+err.Error(), nil, c)
		return
	}
	// 获取角色动态路由
	route := service.GetDynamicRoute(routeSlice)
	response.OK("菜单获取成功", route, c)
}
